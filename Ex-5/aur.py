...
def gen_initX(key,J,r1,r2,pos,plot=False):
    k1, k2, k3 = random.split(key, 3)
    
    theta = random.uniform(k1, (J, 1), minval=0, maxval=2*jnp.pi)
    

    phi = random.uniform(k2, (J, 1), minval=0, maxval=2*jnp.pi)
    radius = jnp.sqrt(random.uniform(k2, (J, 1), minval=0, maxval=1)) * r2
    

    X_main = r1 * jnp.concatenate([jnp.cos(theta), jnp.sin(theta), jnp.zeros_like(theta)], axis=1)
    X_offset = radius * jnp.concatenate([jnp.cos(phi)*jnp.cos(theta), 
